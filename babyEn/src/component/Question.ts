class Question extends eui.Component implements eui.UIComponent {
	...
	// public click: egret.Sound = RES.getRes("click_mp3");
	public isShowAnim: boolean = false;
	// public iconImg: eui.Image;
	public team: eui.Group;
	public team1: eui.Group;
	// public ptgp: eui.Group;
	// public boxGp1: eui.Group;
	// public itemGp: eui.Group;
	public tuopan: eui.Group;
	public chuchuang: eui.Group;
	public chuchuang1: eui.Group;
	...
	public hitArray: Array<any> = [];
	public pointArray1: Array<any> = [{ x: 175, y: 86 }, { x: 0, y: 0 }, { x: 0, y: 178 }, { x: 350, y: 0 }, { x: 350, y: 178 }];
	public pointArray2: Array<any> = [{ x: 175, y: 0 }, { x: 175, y: 178 }, { x: -28, y: 0 }, { x: -28, y: 178 }, { x: 380, y: 0 }, { x: 380, y: 178 }];
	...
	public oldArray: Array<any> = [];
	public animationGroup: eui.Group;
	public btnInfoArray: Array<any> = [];
	public tmpwidth = 74;
	public tmpheight = 78;
	public seeNum = 0;
	public resultNum = -1;
	...
	public constructor(x, y, newtmpar, seeNum, IndexSkin) {
		super();
		this.x = x;
		this.y = y;
		this.skinName = "QuestionSkin";
		this.seeNum = seeNum;
	...
	public resetTime() {
		this.timeNum = egret.setInterval(() => {
			for (let i = 0; i < this.team.numChildren; i++) {
				let tmp = this.team.getChildAt(i) as Box;
				tmp.x = tmp.x + this.tmpx;
				if (tmp.x > 1920) {
				...
				// 	}
				if (!this.isClick && tmp.x >= 642 && this.pts.subNum.value == 1) {
					this.stopAnt();
				}
				// }
			}

			for (let i = 0; i < this.team1.numChildren; i++) {
				let tmp = this.team1.getChildAt(i);
				tmp.x = tmp.x + this.tmpx;
				if (tmp.x >= 1920) {
					tmp.x = -1920;
				}
	...
	private addteam() {
		let tmp = this.pts.nowList[this.pts.nowNum];
	...
	public winAnimate() {
		...
		let tmp = this.tuopan.getChildAt(this.resultList.length) as eui.Image;
		...
		this.animateArray[0] = DragonFun.animationFun(this, this.animateArrayObj[0], () => {
			tmp.visible = true;
			tmp.source = this.resultList[this.resultList.length - 1];
			for (let i = 1; i < this.chuchuang.numChildren; i++) {
				let aaa = this.chuchuang.getChildAt(i) as eui.Image;
				if (aaa.source == tmp.source) {
					this.chuchuang1.getChildAt(Number(aaa.name)).visible = true;
					break;
				}
			}
		});
	...
	private hitTest(e: egret.TouchEvent) {
		...
		let tmp = e.target.parent.parent;
